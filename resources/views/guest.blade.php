...
<div class="bg-[#2D6A4F] text-white px-4 pt-8 pb-14 md:px-16 lg:px-32">
    <div class="flex items-center justify-between mb-3">
        ...
        <a href="/login"
            class="text-sm font-medium text-white border border-white/50 px-4 py-1.5 rounded-full hover:bg-white hover:text-[#2D6A4F] transition">
            Masuk
        </a>
    </div>
    <h1 class="text-2xl font-bold">Temukan Makanan Halal</h1>
    <p class="text-sm opacity-70 mt-1">
        Jelajahi {{ $totalResto }} restoran halal, {{ $totalCertified }} sudah bersertifikat MUI
    </p>
...
<div class="px-4 md:px-16 lg:px-32 mt-4 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
    @foreach(['Semua', 'Pedas', 'Murah'] as $f)
    <a href="/?filter={{ $f }}&search={{ $search ?? '' }}"
...
══════════════════════════════════════════ --}}
@if(isset($restorans) && $restorans !== null)
<div class="px-4 md:px-16 lg:px-32 mt-5 pb-10 space-y-8">
    ...
    <div>
        <div class="flex items-center gap-2 mb-3">
            ...
            <h2 class="font-bold text-gray-800">
                Menu
                @if($search ?? false)<span class="font-normal text-gray-400 text-sm">untuk "{{ $search }}"</span>@endif
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <div onclick="bukaModalMenu(this)"
                ...
                <div class="p-3">
                    ...
                    @if(isset($menu->jarak_km) && $menu->jarak_km < 999)
            ...
            <a href="{{ route('restoran.show', $resto->id_restoran) }}"
               class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition block group">
                ...
                <div class="p-3">
                    ...
                    @if(isset($resto->jarak_km) && $resto->jarak_km < 999)
                    <div class="flex items-center gap-1 mt-1">
                        <div class="w-1.5 h-1.5 rounded-full bg-[#2D6A4F]"></div>
    ...
    <div class="text-center py-16">
        ...
        <a href="/" class="mt-4 inline-block text-sm text-[#2D6A4F] font-medium hover:underline">Kembali ke semua</a>
    </div>
    @endif
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div>
            <div class="flex justify-between items-center mb-3">
                <h2 class="font-bold text-gray-800">🔥 Populer</h2>
...
{{-- ══════════════════════════════════════════
     MODAL DETAIL MENU (SAMA SEPERTI DASHBOARD)
══════════════════════════════════════════ --}}
<div id="modalMenu" class="fixed inset-0 z-50 hidden items-end md:items-center justify-center">
    ...
    <div class="relative bg-white w-full md:w-[480px] md:rounded-3xl rounded-t-3xl overflow-hidden shadow-2xl z-10
        ...
        <div class="p-4 border-t border-gray-100 flex-shrink-0">
            <a href="/login" class="w-full block text-center border border-[#2D6A4F] text-[#2D6A4F] hover:bg-[#2D6A4F] hover:text-white py-3 rounded-2xl font-semibold text-sm transition mb-2">
                Login untuk Favorit
            </a>
